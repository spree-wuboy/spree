module Spree
  class Product < Spree::Base
    ...
    add_search_scope :price_between do |low, high|
      joins(:master => :default_price).where(Spree::Price.arel_table[:amount].in(low..high))
    end

    add_search_scope :master_price_lte do |price|
      joins(:master => :default_price).where(Spree::Price.arel_table[:amount].lteq(price))
    end

    add_search_scope :master_price_gte do |price|
      joins(:master => :default_price).where(Spree::Price.arel_table[:amount].gteq(price))
    end
    ...
    add_search_scope :in_taxon do |taxon|
      joins(:classifications).
        where(Spree::Classification.arel_table[:taxon_id].in(taxon.self_and_descendants.pluck(:id))).
